...
import { Eye, Heart, MessageCircle, Repeat, TrendingUp } from "lucide-react";
...
export default function AnalyticsTab({ userId }: { userId: string }) {
  ...
  if (!stats || posts.length === 0) return <div className="py-8 text-center text-gray-500">No posts yet to analyse.</div>;
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {cards.map((card) => (
          <div key={card.label} className="bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700">
