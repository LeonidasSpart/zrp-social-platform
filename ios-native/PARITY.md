...
| Save a listing | `POST`/`DELETE /api/opportunity/{id}/save`, `GET /api/opportunity/{id}` now returns `alreadySaved` — **FIXED server-side** | ✅ (web now hydrates from `alreadySaved`, was always `false` on load) | ✅ | ✅ same field available to consume | IMPLEMENTED |
