...
| Secure session storage | N/A | httpOnly cookie | EncryptedSharedPreferences | Keychain (`kSecAttrAccessibleAfterFirstUnlock`) | IMPLEMENTED |
| Logout / session teardown | N/A (local + `DELETE /api/push/fcm`) | ✅ | ✅ | ✅ (local; push teardown pending Phase 11) | PARTIAL |
...
| Pull to refresh | N/A | ✅ | ✅ | ✅ | IMPLEMENTED |
...
| Share sheet | N/A (client-side, `zrp.one/post/{id}`) | ✅ | ✅ | ✅ | IMPLEMENTED |
...
| --- | --- | --- | --- | --- | --- |
| Image rendering (single) | N/A | ✅ | ✅ | ✅ | IMPLEMENTED |
| Multi-image gallery (`imageUrls`) | N/A | ✅ | ✅ | ✅ (paged, page dots) | IMPLEMENTED |
| GIF rendering | N/A | ✅ | ✅ | ✅ animated, with the file's own per-frame timing; a single-frame GIF falls back to the still path, which is what it is | IMPLEMENTED |
| Inline video playback | N/A | ✅ | ✅ (ExoPlayer) | ✅ muted, looping, autoplaying at the website's own 0.6 visibility threshold, through **one** shared player rather than one per card; tapping opens the full-screen viewer, where the controls are | IMPLEMENTED |
| Full-screen media viewer | N/A | ✅ | ✅ | ✅ (paged, pinch zoom, AVKit video) | IMPLEMENTED |
...
| Viewer + auto-progression + pause | N/A | ✅ | ✅ | ✅ (tap to step, hold to pause, swipe down to dismiss; video runs its real duration) | IMPLEMENTED |
...
| Purchase flow | N/A **none exists** (price informational, deals close off-platform) | n/a | n/a | n/a | n/a (must never be invented) |
...
| Language (29 languages, `ar` RTL) | client-side preference | ✅ | ✅ | ✅ in-app picker, generated from the web's `SUPPORTED_LANGUAGES` (all 29, verified by `Tools/generate-localizations.py --check`); sets locale and layout direction. The 161-key set of iOS-only accessibility strings with no web counterpart is now translated into all 28 non-English languages too (`Tools/ios-extra-strings.json`'s `translations` block, completeness enforced by the same `--check`), see [iOS localization roadmap](#ios-localization-roadmap-29-language-parity) | IMPLEMENTED |
...
| Author tap is not a click | N/A | ✅ | ✅ | ✅ opening the advertiser's profile is not the billed event, the same line `PostCard` draws between its author link and its body | IMPLEMENTED |
...
| Reactions / replies / edit in a group | N/A | ⬜ | ⬜ | ⬜ **no backend for it**: `GROUP_MESSAGE_INCLUDE` attaches only `sender`, and no route acts on a group message beyond deleting your own. Absent on every platform, not an iOS gap | n/a |
...
| `GAMING` news category | `NewsArticleCategory.GAMING` | ✅ | ✅ | ✅ **fixed here**: the iOS enum never listed it, so a GAMING article decoded as `unknown` and rendered with no category label at all, and the editor could not select it. `newsCategory.gaming` was already translated in all 29 languages | IMPLEMENTED |
...
| Analytics: geography/acquisition/platform/language breakdown | `GET /api/admin/analytics/geography` | ⬜ (Android-only) | ✅ | ✅ a section within the same Analytics screen (not a separate one), fetched with the same range picker: users by current country (bar chart) and by signup-time country, acquisition source, platform, language - each a proportional bucket list capped at the top 10, matching the small-cohort `OTHER` fold and the distinct `UNKNOWN` bucket exactly as the route returns them, with no client-side unfolding. `byRegion` is decoded but not rendered, matching web/Android | IMPLEMENTED |
...
| 17 | Localization (29 languages) + accessibility | ✅ in-app language picker (29 languages, RTL), locale-aware formatting, Dynamic Type pass with a CI rule: 17b (translate the 161 iOS-only accessibility strings beyond English) now done for all 28 non-English languages too, see [iOS localization roadmap](#ios-localization-roadmap-29-language-parity) |
...
## iOS localization roadmap: 29-language parity

**Shared ZRP language target: 29 languages** (`src/lib/translations.ts`
`SUPPORTED_LANGUAGES`): en, fr, de, it, sq, es, ru, ar, zh, tr, id, pt,
ja, ko, hi, nl, pl, ro, cs, hu, sv, da, hr, bg, el, no, sr, bs, mk.

**iOS implementation: 29/29, including the iOS-only string set.**
...
- `ios-native/ZRPSocial/Core/Localization/L10nKeys.swift`'s
  `ZrpLanguage.all` lists all 29 codes, in the same order as web's
  `SUPPORTED_LANGUAGES`, and `LanguagePickerView.swift` renders that
  list directly; nothing is hardcoded to a smaller set.
- All 29 `*.lproj` directories exist under `ios-native/ZRPSocial/Resources/`,
  each with **1,351** keys: the 1,190 shared with web plus the 161
...
- `python3 Tools/generate-localizations.py --check` reports
  **"localizations up to date (29 languages, 1190 shared keys)"**: every
  key sourced from the web dictionary is present, complete and
  regenerated-clean in all 29 languages. The same run's unconditional
  `validate_extra_translations()` step (added in this pass) additionally
  fails the build if any of the 161 iOS-only keys is missing a
  translation, or has a `{placeholder}` mismatch, for any of the 28
...
- RTL: `L10nKey.rightToLeftLanguageCodes` is `["ar"]`, matching web's
  `RTL_LANGUAGES` exactly; no EU Wave 1 or Norwegian/Serbian/Bosnian/
  Macedonian language introduced an RTL requirement, and Arabic's
  iOS-only strings were translated with the same RTL-correct phrasing
  conventions already used for the shared 1,190 keys (no bidi markup
  needed, Arabic script is inherently RTL and SwiftUI mirrors
  automatically).
- `Tools/audit-parity.py` passes: 234 rows marked IMPLEMENTED, every one
  backed by a real iOS call site.
- `Tools/validate-sources.py` passes: 228 Swift files, no hardcoded
  literal strings, braces balanced, plists valid.

### 1. Languages already implemented

All 29, on the full 1,351-key surface (1,190 shared + 161 iOS-only):
...
Bulgarian, Greek, Norwegian, Serbian, Bosnian, Macedonian.
...
**Done.** `ios-native/Tools/ios-extra-strings.json` documents 161
...
translated value for every one of the 161 keys in every one of the 28
non-English languages: the same terminology conventions already
established in each language's shared 1,190-key set were cross-checked
...
None required. `LanguagePickerView.swift` already lists all 29
...
selector work is blocked on the 29-language target.
...
### 8. Final 29-language parity milestone

**Reached, fully.** Both halves of iOS localization are now complete and
CI-enforced: the 1,190 keys shared with web across all 29 languages, and
the 161 iOS-only accessibility/UX strings across all 28 non-English
